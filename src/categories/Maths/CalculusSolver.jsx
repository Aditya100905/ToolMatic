...
export default function EnhancedCalculusSolver({ theme = "light" }) {
  ...
  const adaptiveSimpson = (f, a, b, tolerance) => {
    ...
    const adaptiveHelper = (f, a, b, tolerance, S) => {
      ...
      return (
        adaptiveHelper(f, a, c, tolerance / 2, left) +
        adaptiveHelper(f, c, b, tolerance / 2, right)
      );
    };
  ...
  const findAntiderivativeAdvanced = (expr, variable) => {
    try {
      ...
      const constantMatch = simplified.match(
        /^([+-]?\d+(?:\.\d+)?)\s*\*\s*(.+)$/
      );
      if (constantMatch) {
      ...
      throw new Error(
        `Symbolic integration not available for "${expr}". Try using definite integral with bounds for numerical result.`
      );
    } catch (error) {
  ...
  return (
    <div
      className={`min-h-screen ${currentTheme.bg} transition-all duration-300`}
    >
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-4 sm:gap-6">
            <div className="xl:col-span-3">
              <div
                className={`${currentTheme.card} rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 border`}
              >
                <div className="mb-6 sm:mb-8">
                  <label
                    className={`block text-sm font-semibold ${currentTheme.textSecondary} mb-3 sm:mb-4 flex items-center gap-2`}
                  >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
                  <div className="lg:col-span-1">
                    <label
                      className={`block text-sm font-semibold ${currentTheme.textSecondary} mb-3`}
                    >
                      Variable
                    </label>
                    <input
                  ...
                  <div className="lg:col-span-3">
                    <div className="flex items-center justify-between mb-3">
                      <label
                        className={`block text-sm font-semibold ${currentTheme.textSecondary} flex items-center gap-2`}
                      >
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        {Object.entries(functionCategories).map(
                          ([category, functions]) => (
                            <div key={category}>
                              <h4
                                className={`text-xs font-medium ${currentTheme.textMuted} mb-2 uppercase tracking-wider`}
                              >
                                {category}
                              </h4>
                ...
                <div className="mb-6 sm:mb-8">
                  <label
                    className={`block text-sm font-semibold ${currentTheme.textSecondary} mb-3 flex items-center gap-2`}
                  >
                  ...
                  />
                  <div
                    className={`mt-2 text-xs ${currentTheme.textMuted} flex flex-wrap gap-4`}
                  >
                  ...
                  <div className="mb-6 sm:mb-8">
                    ...
                    {showBounds && (
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label
                            className={`block text-xs font-medium ${currentTheme.textMuted} mb-2`}
                          >
                            Lower Bound
                          </label>
                          <input
                            ...
                            value={bounds.lower}
                            onChange={(e) =>
                              setBounds({ ...bounds, lower: e.target.value })
                            }
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={bounds.upper}
                            onChange={(e) =>
                              setBounds({ ...bounds, upper: e.target.value })
                            }
                            className={`w-full p-3 border rounded-xl ${currentTheme.input} text-center`}
                            placeholder="1"
                          />
                ...
                {showResult && result && (
                  <div
                    className={`p-4 sm:p-6 rounded-xl sm:rounded-2xl ${currentTheme.success} border`}
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                    ...
                    <div
                      className={`p-4 rounded-lg ${currentTheme.result} font-mono text-lg break-all`}
                    >
                      {result}
                    </div>
                    {operation === "integral" &&
                      ...
                      bounds.upper !== "" && (
                        <div
                          className={`mt-3 p-3 rounded-lg ${currentTheme.alternativeFormat} text-sm`}
                        >
                          <div className="font-medium mb-1">
                            Definite Integral:
                          </div>
                          <div className="font-mono">
                            ∫[{bounds.lower}→{bounds.upper}] {expression} d
                            {variable} = {result}
                          </div>
                        </div>
                      )}
            ...
            <div className="xl:col-span-1">
              <div
                className={`${currentTheme.card} rounded-2xl sm:rounded-3xl p-4 sm:p-6 border sticky top-4`}
              >
                <div className="flex items-center justify-between mb-4">
                  <h3
                    className={`font-bold text-lg ${currentTheme.text} flex items-center gap-2`}
                  >
                ...
                {showHistory && (
                  <div className="space-y-3 max-h-96 overflow-y-auto">
                    {history.length === 0 ? (
                      <div
                        className={`text-center py-8 ${currentTheme.textMuted}`}
                      >
                      ...
                      history.map((item) => (
                        ...
                        >
                          ...
                          <div
                            className={`text-sm font-mono ${currentTheme.textSecondary} mb-1 truncate`}
                          >
                            {item.expression}
                          </div>
                          <div
                            className={`text-xs font-mono ${currentTheme.textMuted} truncate`}
                          >
                            = {item.result}
                          </div>
                          <div
                            className={`text-xs ${currentTheme.textMuted} mt-2`}
                          >
                            {item.timestamp}
                          </div>
                        </div>
