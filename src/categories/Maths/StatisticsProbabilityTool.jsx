...
export default function StatisticsProbabilityTool({ theme = "dark" }) {
  ...
  const [normalParams, setNormalParams] = useState({
    mean: 0,
    stdDev: 1,
    x: 0,
  });
  ...
  useEffect(() => {
    if (dataInput.trim()) {
      try {
        ...
        setErrorMessage(
          "Error parsing data. Please enter numbers separated by commas, spaces, or new lines."
        );
      }
    }
  }, [dataInput, activeTab]);
  ...
  const calculateDescriptiveStats = (data) => {
    ...
    try {
      ...
      const geometricMean = data.every((x) => x > 0)
        ? math.exp(math.mean(data.map((x) => math.log(x))))
        : "N/A (requires positive data)";
      const harmonicMean = data.every((x) => x > 0)
        ? data.length / math.sum(data.map((x) => 1 / x))
        : "N/A (requires positive data)";
      ...
      setResults({
        ...
        geometricMean:
          typeof geometricMean === "string"
            ? geometricMean
            : geometricMean.toFixed(4),
        harmonicMean:
          typeof harmonicMean === "string"
            ? harmonicMean
  ...
  const calculateBinomialProbability = () => {
    ...
    try {
      ...
      const cumulativeLower = Array.from(
        { length: k + 1 },
        (_, i) =>
      ...
      setErrorMessage(
        "Error calculating binomial probability: " + error.message
      );
    }
  };
  ...
  const calculateNormalProbability = () => {
    ...
    try {
      ...
      const distribution = Array.from({ length: 100 }, (_, i) => {
        ...
        const density =
          Math.exp(-0.5 * z * z) / (stdDev * Math.sqrt(2 * Math.PI));
  ...
  const calculatePoissonProbability = () => {
    ...
    try {
      const exactProb =
        (Math.exp(-lambda) * Math.pow(lambda, k)) / math.factorial(k);
      let cumulativeLower = 0;
      for (let i = 0; i <= k; i++) {
        cumulativeLower +=
          (Math.exp(-lambda) * Math.pow(lambda, i)) / math.factorial(i);
      }
      ...
      setErrorMessage(
        "Error calculating Poisson probability: " + error.message
      );
    }
  };
  ...
  const handleCorrelationAnalysis = () => {
    try {
      const xValues = correlationData.x
        .split(/[,\s\n]+/)
        .map(Number)
        .filter((num) => !isNaN(num));
      const yValues = correlationData.y
        .split(/[,\s\n]+/)
        .map(Number)
        .filter((num) => !isNaN(num));
      ...
      if (xValues.length < 2) {
        setErrorMessage(
          "At least 2 data points are required for correlation analysis."
        );
        return;
      }
  ...
  const calculateHypothesisTest = () => {
    try {
      if (parsedData.length < 2) {
        setErrorMessage(
          "At least 2 data points are required for hypothesis testing."
        );
        return;
      }
      ...
      if (test === "ttest") {
        ...
        if (alternative === "two-sided") {
          ...
          conclusion =
            testStatistic > criticalValue ? "Reject H₀" : "Fail to reject H₀";
        } else {
          // less
        ...
        if (alternative === "two-sided") {
          ...
          conclusion =
            Math.abs(testStatistic) > criticalValue
              ? "Reject H₀"
              : "Fail to reject H₀";
        } else if (alternative === "greater") {
          pValue = 1 - approximateNormalCDF(testStatistic);
      ...
      setResults({
        ...
        hypothesisTest: {
          ...
          alternativeHypothesis: getAlternativeHypothesisText(
            alternative,
            nullValue
          ),
      ...
      <div className="h-64 mt-6">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            {boxPlotData.outliers &&
              boxPlotData.outliers.map((value, i) => (
                <Bar
                  key={i}
                  dataKey={() => value}
                  fill="red"
  ...
  return (
    <div
      className={`p-4 mt-20 ${themeColors.bg} ${themeColors.text} min-h-screen font-sans`}
    >
      <div className="max-w-6xl mx-auto">
                ...
                {chartType === "histogram" && (
                  <div>
                    ...
                    <input
                      ...
                      max="50"
                      onChange={(e) =>
                        setHistogramBins(parseInt(e.target.value) || 10)
                      }
                    />
                  </div>
                )}
              ...
              <div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className={`p-4 rounded-md ${themeColors.input}`}>
                    ...
                    <p>
                      <span className="font-medium">Mode:</span> {results.mode}
                    </p>
                    <p>
                      <span className="font-medium">Geometric Mean:</span>{" "}
                      {results.geometricMean}
                    </p>
                    <p>
                      <span className="font-medium">Harmonic Mean:</span>{" "}
                  ...
                  <div className={`p-4 rounded-md ${themeColors.input}`}>
                    ...
                    <p>
                      <span className="font-medium">IQR:</span> {results.iqr}
                    </p>
                    <p>
                      <span className="font-medium">CV:</span> {results.cv}
                    </p>
                  </div>
                  ...
                  <div className={`p-4 rounded-md ${themeColors.input}`}>
                    ...
                    <p>
                      <span className="font-medium">Min:</span> {results.min}
                    </p>
                    <p>
                      <span className="font-medium">Q1:</span> {results.q1}
                    </p>
                    <p>
                      <span className="font-medium">Q3:</span> {results.q3}
                    </p>
                    <p>
                      <span className="font-medium">Max:</span> {results.max}
                    </p>
                    <p>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBinomialParams({
                        ...binomialParams,
                        p: parseFloat(e.target.value) || 0.5,
                      })
                    }
                  />
                </div>
                  ...
                  {probabilityType === "binomial" && (
                    <>
                      <div className={`p-4 rounded-md ${themeColors.input}`}>
                        ...
                        <p>
                          <span className="font-medium">
                            P(X = {binomialParams.k}):
                          </span>{" "}
                          {results.exactProbability}
                        </p>
                        <p>
                          <span className="font-medium">
                            P(X ≤ {binomialParams.k}):
                          </span>{" "}
                          {results.cumulativeLowerProbability}
                        </p>
                        <p>
                          <span className="font-medium">
                            P(X ≥ {binomialParams.k}):
                          </span>{" "}
                          {results.cumulativeUpperProbability}
                        </p>
                      </div>
                    ...
                    <>
                      <div className={`p-4 rounded-md ${themeColors.input}`}>
                        ...
                        <p>
                          ...
                          {results.greaterThanX}
                        </p>
                        <p>
                          <span className="font-medium">Z-score:</span>{" "}
                          {results.standardizedZ}
                        </p>
                      </div>
                        ...
                        <BarChart data={results.distribution}>
                          ...
                          <Bar
                            ...
                            {...(probabilityType === "binomial"
                              ? {
                                  fillOpacity: (entry) =>
                                    entry.k === binomialParams.k ? 1 : 0.6,
                                }
                              : {
                                  fillOpacity: (entry) =>
                                    entry.k === poissonParams.k ? 1 : 0.6,
                                })}
                          />
                        </BarChart>
                      )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className={`p-4 rounded-md ${themeColors.input}`}>
                    ...
                    <p>
                      ...
                      {correlationResults.r}
                    </p>
                    <p>
                      <span className="font-medium">
                        Coefficient of Determination (r²):
                      </span>{" "}
                      {correlationResults.rSquared}
                    </p>
                  </div>
                  ...
                  <div className={`p-4 rounded-md ${themeColors.input}`}>
                    ...
                    <p>
                      <span className="font-medium">Slope (b):</span>{" "}
                      {correlationResults.slope}
                    </p>
                    <p>
                      <span className="font-medium">Intercept (a):</span>{" "}
                      {correlationResults.intercept}
                    </p>
                    <p>
                      <span className="font-medium">Regression Equation:</span>{" "}
                      {correlationResults.equation}
                    </p>
                  </div>
                </div>
        ...
        {activeTab === "hypothesisTesting" && (
          <div className={`${themeColors.cardBg} p-6 rounded-lg shadow-lg`}>
            ...
            <div className="mb-6">
              <label className="block mb-2 font-medium">
                Enter Sample Data (numbers separated by commas, spaces, or new
                lines):
              </label>
              <textarea
                className={`w-full px-4 py-2 rounded-md ${themeColors.input} ${themeColors.text} ${themeColors.border} border`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setHypothesis({ ...hypothesis, test: e.target.value })
                  }
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setHypothesis({
                      ...hypothesis,
                      nullValue: parseFloat(e.target.value) || 0,
                    })
                  }
                />
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div className={`p-4 rounded-md ${themeColors.input}`}>
                    ...
                    <p>
                      ...
                      {results.hypothesisTest.test === "ttest"
                        ? "One-sample t-test"
                        : "One-sample z-test"}
                    </p>
                    <p>
                      <span className="font-medium">Null Hypothesis (H₀):</span>{" "}
                      {results.hypothesisTest.nullHypothesis}
                    </p>
                    <p>
                      <span className="font-medium">
                        Alternative Hypothesis (H₁):
                      </span>{" "}
                      {results.hypothesisTest.alternativeHypothesis}
                    </p>
                    <p>
                      <span className="font-medium">
                  ...
                  <div className={`p-4 rounded-md ${themeColors.input}`}>
                    ...
                    <p>
                      ...
                      {results.hypothesisTest.testStatistic}
                    </p>
                    <p>
                      <span className="font-medium">p-value:</span>{" "}
                      {results.hypothesisTest.pValue}
                    </p>
                    <p>
                  ...
                  <div className={`p-4 rounded-md ${themeColors.input}`}>
                    ...
                    <p>
                      ...
                      {results.hypothesisTest.sampleMean}
                    </p>
                    <p>
                      <span className="font-medium">Sample Std. Dev:</span>{" "}
                      {results.hypothesisTest.sampleStdDev}
                    </p>
                    <p>
                      <span className="font-medium">Sample Size:</span>{" "}
                      {results.hypothesisTest.sampleSize}
                    </p>
                    <p>
                      <span className="font-medium">Std. Error:</span>{" "}
                      {(
                        results.hypothesisTest.sampleStdDev /
                        Math.sqrt(results.hypothesisTest.sampleSize)
                      ).toFixed(4)}
                    </p>
                  </div>
