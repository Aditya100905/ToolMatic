...
const ScientificCalculator = ({ theme = "light" }) => {
  ...
  const calculateResult = () => {
    try {
      ...
      expressionToEvaluate = expressionToEvaluate
        ...
        .replace(
          /asin\(/g,
          isRadians ? "Math.asin(" : "(180/Math.PI)*Math.asin("
        )
        .replace(
          /acos\(/g,
          isRadians ? "Math.acos(" : "(180/Math.PI)*Math.acos("
        )
        .replace(
          /atan\(/g,
          isRadians ? "Math.atan(" : "(180/Math.PI)*Math.atan("
        )
        .replace(/\^/g, "**")
        .replace(/×/g, "*")
        .replace(/÷/g, "/");
      ...
      if (
        /[a-zA-Z]/.test(
          expressionToEvaluate.replace(
            /Math\.|sin|cos|tan|log|sqrt|cbrt|asin|acos|atan|PI|E/g,
            ""
          )
        )
      ) {
      ...
      const calculatedResult = evaluate(expressionToEvaluate);
      ...
      setHistory((prev) => [
        {
          expression:
            input + (bracketCount > 0 ? ")".repeat(bracketCount) : ""),
  ...
  const calculatePercentage = () => {
    try {
      ...
      if (lastOperatorIndex === -1) {
        ...
        const percentValue =
          parseFloat(input.substring(lastOperatorIndex + 1)) / 100;

        let newValue;
        if (operator === "+" || operator === "-") {
          // For + and -, calculate percent of the base value
          const base = evaluate(
            baseValue.replace(/×/g, "*").replace(/÷/g, "/")
          );
  ...
  return (
    <div className="flex md:mt-16 mt-20 justify-center items-center w-full px-2 py-4">
      <div
        className="w-full sm:w-4/5 md:w-3/4 lg:w-3/5 xl:w-2/5 rounded-2xl overflow-hidden shadow-2xl"
      ...
      >
        ...
        <div className="w-full rounded-xl" style={styles.display}>
          ...
          <div
            className="flex flex-wrap justify-between text-xs mt-1 sm:mt-2 border-t pt-1 sm:pt-2 border-opacity-20"
          ...
          >
            <div className="flex flex-wrap items-center gap-1 sm:gap-2 mb-1">
              ...
              {shiftMode && (
                <span
                  className="px-1 sm:px-2 py-0.5 sm:py-1 rounded-md text-xs bg-opacity-20 bg-blue-500"
                  style={{
            ...
            <div>
              {bracketCount > 0 && (
                <span
                  className="px-1 sm:px-2 py-0.5 sm:py-1 rounded-md text-xs bg-opacity-20 bg-yellow-500"
                  style={{
                    backgroundColor: isDark
                      ? "rgba(255, 193, 7, 0.2)"
                      : "rgba(255, 193, 7, 0.1)",
              ...
              {memory !== 0 && (
                <span
                  className="px-1 sm:px-2 py-0.5 sm:py-1 rounded-md text-xs bg-opacity-20 bg-purple-500 ml-1"
                  style={{
                    backgroundColor: isDark
                      ? "rgba(156, 39, 176, 0.2)"
                      : "rgba(156, 39, 176, 0.1)",
          ...
          <div style={styles.errorMsg} className="text-xs sm:text-sm">
            {error}
          </div>
        </div>
        ...
        <div className="hidden md:grid grid-cols-5 gap-1 sm:gap-2">
          ...
          {renderButton(
            shiftMode ? "x³" : "x²",
            shiftMode ? handleCube : handleSquare,
            styles.functionButton
          )}
          {renderButton(
            shiftMode ? "cbrt(" : "sqrt(",
            () => handleFunction(shiftMode ? "cbrt" : "sqrt"),
            styles.functionButton
          )}
          ...
          {renderButton(
            shiftMode ? "asin(" : "sin(",
            () => handleFunction(shiftMode ? "asin" : "sin"),
            styles.functionButton
          )}
          {renderButton(
            shiftMode ? "acos(" : "cos(",
            () => handleFunction(shiftMode ? "acos" : "cos"),
            styles.functionButton
          )}
          {renderButton(
            shiftMode ? "atan(" : "tan(",
            () => handleFunction(shiftMode ? "atan" : "tan"),
            styles.functionButton
          )}
          {renderButton(
            shiftMode ? "ln(" : "log(",
            () => handleFunction(shiftMode ? "ln" : "log"),
            styles.functionButton
          )}
