...
const FullURLShortener = ({ theme = "light" }) => {
      ...
      try {
        ...
        if (savedUrls) {
          ...
          if (Array.isArray(parsedUrls)) {
            ...
            console.log(
              "Successfully loaded URL history:",
              parsedUrls.length,
              "items"
            );
          } else {
    ...
    const saveHistory = () => {
      if (urlHistory && Array.isArray(urlHistory)) {
        try {
          ...
          console.log(
            "Saved URL history to localStorage:",
            urlHistory.length,
            "items"
          );
        } catch (err) {
  ...
  const fetchUrlStats = (urlItem) => {
    ...
    setTimeout(() => {
      ...
      // Normalize to 100%
      const browserSum =
        browsers.chrome + browsers.firefox + browsers.safari + browsers.edge;
      ...
      browsers.edge =
        100 - browsers.chrome - browsers.firefox - browsers.safari;
  ...
  const visitUrl = (urlItem) => {
    ...
    setUrlHistory((prevHistory) => {
      const updatedHistory = prevHistory.map((item) =>
        item.id === urlItem.id ? { ...item, clicks: item.clicks + 1 } : item
      );
      return updatedHistory;
    });
  };
  ...
  const URLStats = ({ stats }) => {
    ...
    return (
      <div
        className={`${themeColors.card} ${themeColors.shadow} rounded-lg p-4 mt-4`}
      >
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold">
            Analytics for {stats.url.shortUrl}
          </h3>
          <button
        ...
        <div className="text-center mb-4">
          ...
          <div className={`${themeColors.secondaryText} text-sm`}>
            Total Clicks
          </div>
        </div>
          ...
          <div className="flex items-end h-32 gap-1">
            {stats.dailyData.map((day, index) => (
              <div key={index} className="flex flex-col items-center flex-1">
                ...
                <div className={`${themeColors.secondaryText} text-xs mt-1`}>
                  {day.date}
                </div>
              </div>
            ))}
          ...
          <div>
            ...
            {Object.entries(stats.devices).map(([device, percentage]) => (
              <div key={device} className="mb-2">
                ...
                <div
                  className={`w-full h-2 ${themeColors.statsBar} rounded-full`}
                >
                  <div
  ...
  const HistoryManagementTools = () => (
    <div
      className={`mt-4 p-4 ${themeColors.card} ${themeColors.shadow} rounded-lg`}
    >
  ...
  const URLHistoryTable = () => (
    <div className="overflow-x-auto mt-4">
      <table className="min-w-full">
        <thead>
          <tr
            className={`${themeColors.inputBg} border-t border-b ${themeColors.divider}`}
          >
          ...
          {urlHistory.length === 0 ? (
            ...
            urlHistory.map((item) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-sm">{item.clicks}</td>
                <td className="px-4 py-3">
  ...
  const QRCodeModal = () => (
    ...
    >
      ...
      >
        ...
        <div className="text-center text-sm mb-4 break-all">{shortUrl}</div>
  ...
  return (
    <div
      className={`${themeColors.background} ${themeColors.text} min-h-screen p-4 transition-colors duration-300 mt-20`}
    >
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="mb-6 text-center">
          <h1 className="text-3xl font-bold">URL Shortener</h1>
          <p className={`mt-2 ${themeColors.secondaryText}`}>
        ...
        {viewMode === "create" && (
          <div
            className={`${themeColors.card} ${themeColors.shadow} rounded-lg p-6 mb-6`}
          >
            <div className="mb-4">
            ...
            {shortUrl && (
              <div
                className={`mt-6 p-4 border ${themeColors.divider} rounded-md`}
              >
