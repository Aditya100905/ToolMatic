...
const Qr = ({ theme }) => {
  ...
  const inputBgClass = isDarkTheme ? "bg-gray-700 border-gray-600" : "bg-white border-gray-300";
  const buttonClass = isDarkTheme ? "bg-blue-600 hover:bg-blue-700" : "bg-blue-500 hover:bg-blue-600";
  const secondaryButtonClass = isDarkTheme ? "bg-gray-700 hover:bg-gray-600" : "bg-gray-200 hover:bg-gray-300";
  ...
  const presets = [
    ...
    { label: "vCard", example: "BEGIN:VCARD\nVERSION:3.0\nN:Doe;John\nTEL:123456789\nEMAIL:john@example.com\nEND:VCARD" }
  ];
  ...
  return (
    <div className={`flex flex-col mt-16 items-center justify-center min-h-screen w-full transition-all duration-300 ${
      isDarkTheme ? "bg-black text-white" : "bg-gray-50 text-gray-800"
    }`}>
      <div className={`w-full max-w-5xl p-5 md:p-8 rounded-xl shadow-xl ${bgClass} transition-all duration-300`}>
        
        <div className="flex items-center justify-between mb-6">
          <h1 className={`text-2xl md:text-3xl font-bold ${isDarkTheme ? "text-blue-400" : "text-blue-600"}`}>
            QR Code Generator
          </h1>
          <div className="flex space-x-2">
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg transition ${secondaryButtonClass} flex items-center`}
            >
              <span className="material-icons text-sm mr-1">{previewMode ? "" : ""}</span>
            ...
            {history.length === 0 ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {history.map((item, index) => (
                  ...
                  >
                    <img src={item.url} alt="History QR" className="w-full h-auto mb-1" />
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 flex flex-col gap-6">
            ...
            {/* QR Code Preview */}
            {qrCode && (
              <div className={`flex flex-col items-center p-6 rounded-lg ${panelBgClass}`}>
                <div className={`p-6 rounded-lg ${previewMode ? panelBgClass : "bg-white"}`}>
                  <img 
                    ref={qrImageRef}
                    src={qrCode} 
                    alt="Generated QR Code" 
                    className="max-w-full h-auto shadow-lg"
                    style={{ width: `${previewMode ? size : Math.min(300, size)}px`, maxWidth: "100%" }}
                  />
                </div>
                <div className="flex flex-wrap justify-center gap-3 mt-6">
                  ...
                  >
                    <span className="material-icons text-sm mr-1">{copySuccess ? "" : ""}</span>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {[
                          ...
                          >
                            <div className="w-full h-6 mb-1 rounded" style={{ background: preset.fg }}></div>
                            {preset.name}
                          </button>
                        ))}
                      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm opacity-80">
                        Higher error correction makes QR codes more reliable even when partially damaged or obscured,
                        but results in a more complex code with more modules.
                      </p>
                    </div>
                    
                    <div className="p-3 rounded-lg bg-opacity-30 border-l-4 border-blue-500 text-sm">
                      <span className="font-medium block mb-1">QR Code Tips:</span>
                      <ul className="list-disc list-inside space-y-1 opacity-80">
                        ...
                        <li>Shorter content will produce simpler, more reliable codes</li>
                      </ul>
                    </div>
                  </div>
                </div>
      ...
      <div className="mt-6 text-center text-sm opacity-70 px-4 max-w-lg">
        ...
        <p className="mt-1">{navigator.share ? "Use the Share button to share directly to other apps" : "Download your QR code to share it with others"}</p>
      </div>
    </div>
  );
};
