...
import { 
  Clipboard, ArrowDownUp, Code, FileText, Eye, EyeOff, 
  X, RefreshCw, Download, Upload,
  CheckCircle
} from 'lucide-react';
...
export default function MarkdownHtmlConverter({ theme = 'dark' }) {
  ...
  const [conversionStatus, setConversionStatus] = useState({ success: true, message: '' });
  ...
  const getTurndownService = () => {
    ...
    tds.addRule('tables', {
      ...
      replacement: function(content, node) {
        ...
        if (rows.length > 0) {
          markdownTable += '| ' + Array.from(rows[0].cells)
          ...
          for (let i = 1; i < rows.length; i++) {
            markdownTable += '| ' + Array.from(rows[i].cells)
              .map(cell => cell.textContent.trim())
              .join(' | ') + ' |\n';
          }
        }
  ...
  const handleModeToggle = () => {
    setMode(mode === 'markdown-to-html' ? 'html-to-markdown' : 'markdown-to-html');
    setInputText(outputText);
    setOutputText(inputText);
    showNotification(`Switched to ${mode === 'markdown-to-html' ? 'HTML to Markdown' : 'Markdown to HTML'} mode`);
  };
...
| col 1  | col 2 | col 3 |`;
      ...
      return `<h1>Welcome to the HTML Editor</h1>
...
<table>
  ...
  // Styles with enhanced responsive considerations
  const styles = {
    container: {
    ...
    header: {
      textAlign: 'center',
      marginBottom: ['xs', 'small'].includes(screenSize) ? '0.75rem' : '2rem',
      width: '100%',
      maxWidth: '1200px',
    },
    title: {
    ...
    buttonBar: {
      display: 'flex',
      flexWrap: 'wrap',
      gap: ['xs', 'small'].includes(screenSize) ? '0.5rem' : '0.75rem',
      marginBottom: '1.5rem',
      justifyContent: 'space-between',
      flexDirection: ['xs', 'small', 'medium'].includes(screenSize) ? 'column' : 'row',
    },
    buttonGroup: {
      display: 'flex',
      gap: screenSize === 'xs' ? '0.25rem' : '0.5rem',
      flexWrap: 'wrap',
      justifyContent: ['xs', 'small', 'medium'].includes(screenSize) ? 'space-between' : 'flex-start',
    },
    button: {
      display: 'flex',
      alignItems: 'center',
    ...
    secondaryButton: {
      backgroundColor: theme === 'dark' ? c.surface2 : c.surface,
      color: c.text,
    ...
    panels: {
      ...
      gap: ['xs', 'small'].includes(screenSize) ? '0.75rem' : '1.5rem',
      marginBottom: '1.5rem',
    },
    panel: {
      display: 'flex',
    ...
    panelHeader: {
      ...
      justifyContent: 'space-between',
      alignItems: 'center',
      flexWrap: ['xs', 'small', 'medium'].includes(screenSize) ? 'wrap' : 'nowrap',
      gap: '0.5rem',
    },
    panelTitle: {
    ...
    notification: {
      ...
      backgroundColor: notification.type === 'success'
        ? (theme === 'dark' ? 'rgba(16, 185, 129, 0.9)' : 'rgba(16, 185, 129, 0.9)')
        : notification.type === 'error'
          ? (theme === 'dark' ? 'rgba(239, 68, 68, 0.9)' : 'rgba(239, 68, 68, 0.9)')
          : (theme === 'dark' ? 'rgba(59, 130, 246, 0.9)' : 'rgba(59, 130, 246, 0.9)'),
      color: 'white',
      ...
      gap: '0.5rem',
      maxWidth: '90%',
      animation: notification.show ? 'slideIn 0.3s ease-out' : 'none',
    ...
    <div style={styles.container}>
      {notification.show && (
        <div style={styles.notification}>
          {notification.type === 'success' && <CheckCircle size={iconSize} />}
          {notification.type === 'error' && <X size={iconSize} />}
          {notification.type === 'info' && <RefreshCw size={iconSize} />}
      ...
      <div style={styles.header}>
        <h1 style={styles.title}>
          {mode === 'markdown-to-html' ? 'Markdown to HTML Converter' : 'HTML to Markdown Converter'}
        </h1>
        <p style={styles.subtitle}>
          Simple and powerful bidirectional converter with live preview
        </p>
      </div>
        ...
        {inputText && (
          <div style={{display: 'flex', justifyContent: 'flex-end', marginBottom: '0.75rem'}}>
            <div style={styles.statusIndicator}>
              {conversionStatus.success ? <CheckCircle size={iconSize - 2} /> : <X size={iconSize - 2} />}
              {conversionStatus.success ? 'Conversion successful' : 'Conversion failed'}
            </div>
          </div>
        )}
            ...
            <div style={styles.panelHeader}>
              <h2 style={styles.panelTitle}>
                {mode === 'markdown-to-html' 
                  ? <><FileText size={iconSize} />Markdown</>
                  : <><Code size={iconSize} />HTML</>
                }
              </h2>
              <input
                type="file"
        ...
        {mode === 'markdown-to-html' && outputText && (
          <div style={styles.previewPanel}>
            <div style={styles.panelHeader}>
              ...
              >
                {showPreview 
                  ? <><EyeOff size={iconSize} />{screenSize === 'xs' ? 'Hide' : 'Hide Preview'}</>
                  : <><Eye size={iconSize} />{screenSize === 'xs' ? 'Show' : 'Show Preview'}</>
                }
              </button>
            </div>
            {showPreview && (
      ...
      <footer style={styles.footer}>
        <p>Markdown-HTML Converter • Simple bidirectional conversion tool</p>
      </footer>
    </div>
  );
}