...
// Comprehensive power conversion rates (all standardized to Watts)
const powerConversionRates = {
  watt: 1,
  kilowatt: 1000,          // 1 kW = 1000 W
  megawatt: 1e6,           // 1 MW = 1,000,000 W
  horsepower: 745.7,       // 1 hp ≈ 745.7 W
  btuPerHour: 0.29307107,   // 1 BTU/h ≈ 0.29307107 W
};
...
const unitLabels = {
  ...
  btuPerHour: "BTU per hour (BTU/h)",
};

const PowerConverter = ({ theme }) => {
  ...
  const convertPower = (value, fromUnit, toUnit) => {
    const valueInWatts = parseFloat(value) * powerConversionRates[fromUnit];
    const convertedValue = valueInWatts / powerConversionRates[toUnit];
  ...
  const handleConvert = () => {
    ...
    try {
      const result = convertPower(parseFloat(inputValue), fromUnit, toUnit);
      ...
      const conversionRatio = convertPower(1, fromUnit, toUnit);
      setFormula(
      ...
      <div className={`w-full max-w-4xl rounded-2xl p-6 sm:p-10 ${themeStyles.card}`}>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
          <div className="flex flex-col w-full">
            ...
            >
              {Object.keys(powerConversionRates).map((unit) => (
