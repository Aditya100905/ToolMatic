...
// Comprehensive energy conversion rates (all standardized to Joules)
const energyConversionRates = {
  joule: 1,
  kilojoule: 1000,
  calorie: 4.184,       // thermochemical calorie
  kilocalorie: 4184,    // food calorie (kcal)
  wattHour: 3600,
  kilowattHour: 3.6e6,
...
const EnergyConverter = ({ theme }) => {
  ...
  const convertEnergy = (value, fromUnit, toUnit) => {
    const valueInJoules = parseFloat(value) * energyConversionRates[fromUnit];
    const convertedValue = valueInJoules / energyConversionRates[toUnit];
  ...
  const handleConvert = () => {
    ...
    try {
      const result = convertEnergy(parseFloat(inputValue), fromUnit, toUnit);
      ...
      const conversionRatio = convertEnergy(1, fromUnit, toUnit);
      setFormula(
      ...
      <div className={`w-full max-w-4xl rounded-2xl p-6 sm:p-10 ${themeStyles.card}`}>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
          <div className="flex flex-col w-full">
            ...
            >
              {Object.keys(energyConversionRates).map((unit) => (
