...
const Navbar = ({ onSearch }) => {
  ...
  const { searchTerm, setSearchTerm, searchFocused, setSearchFocused } =
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-3">
        ...
        <div className="hidden sm:flex items-center justify-between">
          ...
          <div className="flex-shrink-0 mr-2 lg:mr-4">
            <Link
              to="/"
              className="flex items-center space-x-2 md:space-x-3 text-xl md:text-2xl font-bold tracking-wide hover:opacity-90 transition-all duration-300 group"
            >
          ...
          {/* Navigation Links - Center (responsive spacing) */}
          <div className="flex justify-center flex-grow max-w-md lg:max-w-lg mx-auto">
            <ul className="flex sm:space-x-1 md:space-x-2 lg:space-x-8">
              {[
                { name: "Home", path: "/" },
          ...
          <div className="flex items-center space-x-1 sm:space-x-1 md:space-x-2 lg:space-x-4">
            {/* Search Bar - Desktop & Tablet (Only for utilities page) */}
            {isSearchVisible && (
              <div className="sm:w-32 md:w-48 lg:w-64 xl:w-72">
                <form onSubmit={handleSearchSubmit} className="group">
                  <div className="relative">
                    <input
                      ...
                      className={`w-full py-1 md:py-1.5 lg:py-2 pl-2 md:pl-3 pr-6 sm:pr-6 md:pr-8 lg:pr-16 rounded-full border ${
                        isDark
                          ? "bg-gray-800 border-gray-700 text-white focus:bg-gray-700"
                          : "bg-gray-100 border-gray-200 text-gray-800 focus:bg-white"
                      } focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 text-xs sm:text-xs md:text-sm lg:text-base`}
                    />
                    {/* Keyboard shortcut - only on larger screens */}
                    <div className="absolute right-8 top-1/2 transform -translate-y-1/2 hidden lg:block">
                      <kbd
                        className={`px-1.5 mr-5 py-0.5 text-xs rounded ${
                          isDark
                    ...
                    <button
                      type="submit"
                      className={`absolute right-1 sm:right-1 md:right-2 top-1/2 transform -translate-y-1/2 p-1 lg:p-1.5 rounded-full ${
                        isDark
                    ...
                    >
                      <Search
                        size={14}
                        className="sm:w-3 sm:h-3 md:w-4 md:h-4 lg:w-4 lg:h-4"
                      />
                    </button>
                  </div>
                </form>
            ...
            <button
              onClick={toggleTheme}
              className={`p-1 sm:p-1 md:p-1.5 lg:p-2 rounded-full transition-all duration-300 hover:scale-110 ${
                isDark
                  ? "bg-gray-800 hover:bg-gray-700 hover:ring-1 hover:ring-gray-600"
                  : "bg-gray-100 hover:bg-gray-200 hover:ring-1 hover:ring-gray-300"
              }`}
            ...
            >
              {isDark ? (
                <Sun
                  size={16}
                  className="sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-5 lg:h-5 text-yellow-300"
                />
              ) : (
                <Moon
                  size={16}
                  className="sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-5 lg:h-5 text-blue-500"
                />
              )}
            </button>
          </div>
        ...
        <div className="flex sm:hidden items-center justify-between">
          ...
          <Link
            to="/"
            className="flex items-center space-x-2 text-xl font-bold tracking-wide hover:opacity-90 transition-all duration-300 group"
          >
            <div className="relative overflow-hidden rounded-full p-1 group-hover:shadow-md transition-all duration-300">
              <img
                src={toolmatic}
      ...
      {/* Mobile Menu */}
      <div
        className={`mobile-menu-container sm:hidden transition-all duration-300 ${
          isOpen
            ? "max-h-[100vh] border-t border-b opacity-100"
            : "max-h-0 opacity-0 overflow-hidden"
        } ${
          isDark ? "bg-[#222222] border-gray-800" : "bg-gray-50 border-gray-200"
        }`}
      >
